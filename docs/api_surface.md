...
Payments represent charges processed against subscribers. Merchants can initiate refunds via the API.

### `POST /api/payments/{payment_id}/refund` — Refund Payment

Refunds a successful payment transaction. Only payments with status `succeeded` and a valid Nomba transaction ID can be refunded.
...
**Error:** `400 Bad Request` — Only succeeded payments can be refunded / missing Nomba transaction ID.  
**Error:** `404 Not Found` — Payment record not found.

---

## Customer Portal

The portal is a subscriber-facing UI. Merchants generate portal URLs and give them to their customers.
...
These routes serve HTML pages or API actions and are accessed directly by the subscriber via the portal link:
...
| `/api/portal/{sub_id}/resume` | POST | Resume a subscription scheduled for cancellation |
| `/api/portal/{sub_id}/change-plan` | POST | Schedule a plan switch for the end of the period |
...
| `POST` | `/api/subscriptions/{sub_id}/portal-link` | API Key | Portal |
| `POST` | `/api/payments/{payment_id}/refund` | API Key | Payments |
...
| `POST` | `/api/portal/{sub_id}/resume` | Portal Token | Portal |
| `POST` | `/api/portal/{sub_id}/change-plan` | Portal Token | Portal |
