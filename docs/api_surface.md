...
### `POST /api/subscriptions/{sub_id}/portal-link` — Generate Portal Link *(planned)*

> [!NOTE]
> This endpoint does not exist in code yet. It is the planned way for merchants to programmatically generate time-limited, token-secured portal URLs for their subscribers.
...
### Current Portal Routes (Subscriber-Facing, Not API-Key Protected)
...
> [!WARNING]
> Portal routes are currently unauthenticated — anyone with the `sub_id` can access them. The planned `portal-link` endpoint above will add a short-lived token for secure access.
...
| Event | Fires When |
|-------|------------|
| `subscription.activated` | First payment succeeds (trial → active, or initial → active) |
| `subscription.cancelled` | Subscription cancelled by merchant or subscriber |
| `subscription.suspended` | All dunning retries exhausted |
| `subscription.reactivated` | Subscriber updates card + charge succeeds |
