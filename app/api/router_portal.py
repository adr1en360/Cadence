...
async def update_card(sub_id: str, db: Session = Depends(get_db)):
    ...
    # Small ₦100 card tokenization validation authorization charge
    ...
    try:
        ...
        # Build callback redirect URL back to portal page
        callback_url = f"https://localhost:8000/portal/{sub.id}"
...
def cancel_portal_subscription(sub_id: str, db: Session = Depends(get_db)):
    sub = db.query(Subscription).filter(Subscription.id == sub_id).first()
    if not sub:
        raise HTTPException(status_code=404, detail="Subscription not found")
